...
export function raycastForVSCodiumURI(uri: string) {
  return `vscodium://tonka3000.raycast/${uri}`;
}

export async function openURIInVSCodium(uri: string) {
  await open(raycastForVSCodiumURI(uri));
}
