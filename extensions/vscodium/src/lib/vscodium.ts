...
export function getVSCodiumCLIFilename(): string {
  return "/Applications/VSCodium.app/Contents/Resources/app/bin/codium";
}

export class VSCodiumCLI {
...
export function getVSCodiumCLI(): VSCodiumCLI {
  return new VSCodiumCLI(getVSCodiumCLIFilename());
}
...
export async function getLocalExtensions(): Promise<Extension[] | undefined> {
  const extensionsRootFolder = path.join(os.homedir(), ".vscode-oss/extensions");
