...
export function useRecentEntries() {
  ...
  async function removeEntry(entry: EntryLike) {
    ...
    try {
      ...
      await revalidate();
      showToast(Toast.Style.Success, "Entry removed", "Restart VSCodium to sync the list in VSCodium (optional)");
    } catch (error) {
    ...
    try {
      if (
        ...
        showToast(
          Toast.Style.Success,
          "All entries removed",
          "Restart VSCodium to sync the list in VSCodium (optional)"
        );
      }
    } catch (error) {
...
function getPath() {
  return `${homedir()}/Library/Application Support/VSCodium/User/globalStorage/state.vscdb`;
}

async function saveEntries(entries: EntryLike[]) {
  const data = JSON.stringify({ entries });
  const escapedData = data.replace(/'/g, "''");
  const query = `INSERT INTO ItemTable (key, value) VALUES ('history.recentlyOpenedPathsList', '${escapedData}') ON CONFLICT(key) DO UPDATE SET value = excluded.value;`;
